		...
		return this.each(function() {
			...
			function getEmbed(){
				...
				.append(
					...
					$('<br>'),
					$('<span>').html( 
						'Also production library urls should be used, more info on <a href="http://html5video.org/wiki/Vidiun_HTML5_Configuration#Controlling_the_HTML5_library_version_for_.com_uiConf_urls">' + 
			...
			function getUiConfConfig(){
				...
				return $('<div />').append( 
						...
						$('<span>UiConf XML can be inserted via <a target="top" href="http://www.vidiun.org/modifying-vdp-editing-uiconf-xml">VMC api</a>:</span>') 
					);
			}
			/**
			 * player studio tab
			...
			$.getJSON( request, function( data ){
				...
				var settingTabHtml = ( showSettingsTab ) ? 
						...
						'<i class="vpcicon-integrate"></i>Integrate</a></li>' :
						'';
				$( _this ).empty().append(
					$('<div />')
					.css({
