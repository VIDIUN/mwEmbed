...
if( !window.QUnit ){
	...
	if( typeof mw != 'undefined' && mw.getConfig( 'Vidiun.PageGoogleAalytics' ) ) {
		var _gaq = _gaq || [];
		_gaq.push(['_setAccount', mw.getConfig( 'Vidiun.PageGoogleAalytics' ) ]);
...
if( typeof vWidget != 'undefined' && vWidget.addReadyCallback ){
	...
	vWidget.addReadyCallback( function( pId ){
		$( '#' + pId )[0].vBind("mediaReady.pTimeReady", function(){
			if( alreadyRun ){
				return ;
			}
			alreadyRun = true;
			...
			$('body').append( '<div class="vdocPlayerRenderTime" style="clear:both;"><span style="font-size:11px;">player ready in:<i>' + ( new Date().getTime() - vdocPlayerStartTime )/1000 + '</i> seconds</span></div>');
