			...
			if ( this.playerIsReady && this.playlistSet.length > 1 ) {
				...
				if ( this.getComponent().find( ".playlistSelector" ).length == 0 ) { // UI wasn't not created yet
					...
					$.each( this.playlistSet, function ( i, el ) {
						...
						if ( _this.getLayout() === "vertical" ) {
							_this.getComponent().find( ".playlistSelector" ).append( '<br><div data-index="' + i + '" class="playlistItem"><span class="v-playlistTitle"> ' + el.name + '</span><br><span class="v-playlistDescription multiplePlaylists">' + numOfClips + ' ' + gM( 'mwe-embedplayer-videos' ) + '</span></div>' );
						} else {
							_this.getComponent().find( ".playlistSelector" ).append( '<div data-index="' + i + '" class="playlistItem v-horizontal"><span class="v-playlistTitle"> ' + el.name + '</span><br><span class="v-playlistDescription multiplePlaylists">' + numOfClips + ' ' + gM( 'mwe-embedplayer-videos' ) + '</span></div>' );
						}
					} );
