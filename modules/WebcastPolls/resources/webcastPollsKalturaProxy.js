(function (mw, $) {
    ...
    mw.webcastPolls.WebcastPollsVidiunProxy = mw.VBasePlugin.extend({
        ...
        isErrorResponse : function(result)
        {
            ...
            if ($.isArray(result))
            {
                ...
                {
                    hasError = hasError || (response && response.objectType && response.objectType === "VidiunAPIException");
                });
            }else
            {
                hasError = (result && result.objectType && result.objectType === "VidiunAPIException");
            }
        ...
        getUserVote : function(pollId, userId)
        {
            ...
            if (userId && pollId) {
                ...
                _this.getVidiunClient().doRequest(request, function (response) {
                    if (!_this.isErrorResponse(response)) {
                        ...
                        _this.log("rejecting request due to error from vidiun api server");
                        defer.reject();
                    }

                }, false, function (reason) {
                    _this.log("rejecting request due to error from vidiun api server with reason " + (reason ? JSON.stringify(reason) : ''));
                    defer.reject({});
                });
            }else
            {
		...
		transmitVoteUpdate : function(metadataId, userId, selectedAnswer,pollId)
		{
            ...
            if (pollId && userId && selectedAnswer) {
                ...
                _this.getVClient().doRequest(vote, function (result) {
