...
class vidiunIframeClass {
	...
	<title>Vidiun Embed Player iFrame</title>
	<style type="text/css">
		html,body,video {
			width: 100%;
			height: 100%;
...
HTML;
	...
	function outputVidiunModules(){
		...
		foreach( $vidiunSupportModules as $name => $module ){
			...
			if( isset( $module[ 'vidiunPluginName' ] ) ){
				if( is_array( $module[ 'vidiunPluginName' ] ) ){
					...
					$this->addModuleTemplate( $module, $playerConfig['plugins'][ $module[ 'vidiunPluginName' ] ] );
					$moduleList[] = $name;
				}
			}
		}
...
HTML;
	...
	function getVidiunIframeScripts(){
		global $wgMwEmbedVersion, $wgVidiunApiFeatures;
		ob_start();
		?>
