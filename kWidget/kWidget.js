	...
	var vWidget = {
			...
			if (!mw.getConfig( "Vidiun.SupressNonProductionUrlsWarning", false )) {
				...
				if (
					...
					if (console && console.error) {
						console.error("Error: Using non-prodcution version of vidiun player library. Please see http://knowledge.vidiun.com/production-player-urls")
					}
				}
			}
		...
		thumbEmbed: function (targetId, settings, forceDownload) {
			...
			elm.innerHTML = '' +
				...
				'<button class="vWidgetCentered vWidgetPlayBtn" ' + 'id="' + targetId + '_playBtn" >' +
				'<span class="vWidgetAccessibilityLabel">' + 'Play video content' + '</span></button>' +
		...
		isMissingUiConfJs: function (playerList) {
			...
			if (playerList.length == 0 || !mw.getConfig('Vidiun.EnableEmbedUiConfJs') ||
		...
		getDownloadLink: function (settings) {
			...
			var downloadUrl = baseUrl + 'modules/VidiunSupport/download.php/wid/' + settings.wid;
			...
			var vs = settings.vs;
			if (vs) {
				downloadUrl += '/?vs=' + vs + flashVarsString;
			} else {
		...
		getVidiunThumbUrl: function (settings) {
			...
			if (settings.vs) {
				vidParams+= '/vs/' + settings.vs;
			}
			if (settings.flashvars && settings.flashvars.vs) {
				vidParams+= '/vs/' + settings.flashvars.vs;
			}
			var flashVars = {};
